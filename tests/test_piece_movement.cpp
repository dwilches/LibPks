...
TEST_CASE("All pieces move out of home") {
    ...
    REQUIRE(gameState.piecesByPlayer[PksColor::Yellow] == std::vector{-1, -1, -1, -1}); // all at home
    ...
    REQUIRE(gameState.piecesByPlayer[PksColor::Yellow] == std::vector{0, 0, 0, 0}); // all at start

    // Next player's turn
    REQUIRE(gameState.currentPlayer == PksColor::Red);

    // Next player is still at home
    REQUIRE(gameState.piecesByPlayer[PksColor::Red] == std::vector{-1, -1, -1, -1});
}
...
TEST_CASE("Pieces move according to dice value (same piece moves both dice)") {
    ...
    auto gameState = game.start(initialBoard);
    REQUIRE(gameState.currentPlayer == PksColor::Yellow);
    REQUIRE(gameState.piecesByPlayer[PksColor::Yellow] == std::vector{0, 10, 20, 30});
    ...
    gameState = game.useDice(6, 1); // Should move from 10 to 16
    REQUIRE(gameState.piecesByPlayer[PksColor::Yellow] == std::vector{0, 16, 20, 30});

    gameState = game.useDice(3, 1); // Should move from 16 to 19
    REQUIRE(gameState.piecesByPlayer[PksColor::Yellow] == std::vector{0, 19, 20, 30});
...
TEST_CASE("Pieces move according to dice value (different pieces move)") {
    ...
    gameState = game.useDice(6, 2); // Should move from 20 to 26
    REQUIRE(gameState.piecesByPlayer[PksColor::Yellow] == std::vector{0, 10, 26, 30});

    gameState = game.useDice(3, 1); // Should move from 10 to 13
    REQUIRE(gameState.piecesByPlayer[PksColor::Yellow] == std::vector{0, 13, 26, 30});
...
TEST_CASE("Pieces move according to dice value (doubles test)") {
    ...
    game.rollDice();
    game.useDice(6, 0);
    gameState = game.useDice(6, 1);
    REQUIRE(gameState.piecesByPlayer[PksColor::Yellow] == std::vector{6, 16, 20, 30});
    ...
    game.rollDice();
    game.useDice(5, 1);
    gameState = game.useDice(5, 2);
    REQUIRE(gameState.piecesByPlayer[PksColor::Yellow] == std::vector{6, 21, 25, 30});
    ...
    REQUIRE(gameState.piecesByPlayer[PksColor::Yellow] == std::vector{8, 23, 25, 30});
...
TEST_CASE("Pieces move according to dice value (all players move)") {
    ...
    game.rollDice();
    game.useDice(1, 0);
    gameState = game.useDice(2, 1);
    REQUIRE(gameState.piecesByPlayer[PksColor::Yellow] == std::vector{1, 2, 0, 0});

    // Next player rolls a dice
    mockDiceRoller.setNextRandomValues(5, 3);
    game.rollDice();
    game.useDice(5, 1);
    gameState = game.useDice(3, 2);
    REQUIRE(gameState.piecesByPlayer[PksColor::Red] == std::vector{0, 5, 3, 0});

    // Next player rolls a dice (doubles)
    mockDiceRoller.setNextRandomValues(3, 3);
    game.rollDice();
    game.useDice(3, 0);
    gameState = game.useDice(3, 1);
    REQUIRE(gameState.piecesByPlayer[PksColor::Green] == std::vector{3, 3, 0, 0});
    ...
    game.useDice(2, 0);
    gameState=game.useDice(3, 1);
    REQUIRE(gameState.piecesByPlayer[PksColor::Green] == std::vector{5, 6, 0, 0});

    // Last player rolls a dice
    mockDiceRoller.setNextRandomValues(6, 3);
    game.rollDice();
    game.useDice(3, 3);
    gameState=game.useDice(6, 3);
    REQUIRE(gameState.piecesByPlayer[PksColor::Blue] == std::vector{0, 0, 0, 9});
